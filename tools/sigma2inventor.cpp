...
int main(int argc, char **argv) {
  ...
  while (in >> ct.s >> ct.sigma >> ct.tau) {
    ...
    pt2 = c.pointAt(ct.tau);
		contacts.push_back(CContact(pt2,(pt1+pt2)*.5,pt1));
		*/
  }
  in.close();
  ...
  // Sort contacts, start with first. then look for the closest
	// midpoint, remove it from list and go on.
	/*
	vector<CContact> csort;
	csort.push_back(contacts[0]);
	contacts.erase(contacts.begin());

  while (contacts.size()>0) {
    // look for closest to last in csort
		vector<CContact>::iterator closest = contacts.begin();
		float d = (closest->p[1] - csort.back().p[1]).norm();
		for (vector<CContact>::iterator it=contacts.begin()+1;it!=contacts.end();++it) {
			float d2 = (it->p[1]-csort.back().p[1]).norm();
			if (d2 < d) {
        closest = it;
				d = d2;
			}
		}
		csort.push_back(*closest);
		contacts.erase(closest);
	}
	contacts = csort;
	csort.clear();
  */
