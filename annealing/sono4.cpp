...
void Smooth(Curve<Vector4> &c, int stencil) {

}

void Shrink(Curve<Vector4> &c,float factor) {
  for (int i=0;i<c.nodes();++i)
    c[i].setPoint(c[i].getPoint()*factor);
}

float GetLength(Curve<Vector4> &c) {
  float L = 0;
  for (int i=0;i<c.nodes();++i)
...
void SavePkf(Curve<Vector4> &c, int freq = 50) {
  static int count = 0;
  char buf[16];
  if (count%freq==0) {
...
void ShiftNodes(Curve<Vector4> &c, float scalefactor) {
  int dir = rand()%2;
  Vector4 e1, e2, dummy;
  float n1, n2;
  for (vector<Biarc<Vector4> >::iterator it=c.begin();it!=c.end();it++) {
 ...
 */
void FindNeighbours(Curve<Vector4> &c, float D, int skip, float eps, int m, int **nn) {
	cout << "FIND\n";
...
int RemoveOverlaps(Curve<Vector4> &c, int N, int m, int **nn, float D, float delta
#ifndef NNeighbour
, int skip
#endif
) {
...
void ControlLeashes(Curve<Vector4> &c,float l) {
  /*
     c.computeTangents();
     c.make_default();
     c.resample(c.nodes());
